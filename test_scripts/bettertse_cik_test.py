"""
BetterTSE 时间序列编辑测试框架 - CiK风格增强版
===============================================

完整实现以下功能：
1. 真实CSV数据读取（ETTh1等数据集）
2. CiK风格物理变化注入机制（因果上下文 + 规则约束）
3. 真实LLM API调用生成模糊描述
4. 完整评估指标：t-IoU、分区MSE/MAE（Editability/Preservability）

借鉴《Context is Key》(CiK)基准测试的核心设计：
- 因果关系的显式化 (Causal & Future Context)
- 强制规则约束 (Instruction Following)
- 支持构建"干扰项测试" (Distractor Evaluation)
...
logger = logging.getLogger("BetterTSE_CiK_Evaluator")
...
class ChangeConfig:
    ...
    direction: str = "up"
    description: str = ""
    causal_context: str = ""  # CiK风格：因果上下文
    distractor_text: str = ""  # CiK风格：干扰项文本
...
class CiKStyleDataSynthesizer:
    """
    借鉴 Context is Key (CiK) 逻辑的时序编辑数据合成器
    
    核心设计理念：
    1. 因果关系的显式化 (Causal & Future Context)
    2. 强制规则约束 (Instruction Following)
    3. 支持构建"干扰项测试" (Distractor Evaluation)
    """
    
    TASK_TYPES = [
        "heatwave_overload",           # 热浪导致高负载
        "sensor_maintenance",          # 传感器维护掉线
        "equipment_failure",           # 设备故障
        "scheduled_shutdown",          # 计划停机
        "load_balancing",              # 负载均衡调整
        "anomaly_injection",           # 异常注入
        "trend_reversal",              # 趋势反转
    ...
    def task_heatwave_overload(
        ...
        """
        场景1: 热浪导致高负载 (借鉴CiK的ElectricityIncreaseInPredictionTask)
        
        物理逻辑：突发热浪导致特定时间段内电力负荷激增至原来的N倍
        
        Args:
            df_source: 数据源DataFrame
            start_idx: 数据起始索引
            start_step: 变化起始步骤
            duration: 持续时间
            multiplier: 放大倍数
            include_distractor: 是否包含干扰项
            
        Returns:
            Tuple[base_ts, target_ts, mask_gt, prompt, gt_params]
        """
        if start_step is None:
            start_step = self.rng.randint(int(self.seq_len * 0.3), int(self.seq_len * 0.5))
        if duration is None:
            duration = self.rng.randint(10, 25)
        if multiplier is None:
            multiplier = self.rng.uniform(1.5, 3.0)
        ...
        prompt = (
            ...
            f"According to the meteorological log, a severe heatwave struck the city starting at time step {start_step} "
            f"and lasted for {duration} steps. During this specific window, the {feature_name} experienced "
        ...
        if include_distractor:
            distractor_events = [
                f"There was also a minor rain shower at step {self.rng.randint(0, start_step - 5)}, but it had no effect on the system.",
                f"A small voltage fluctuation occurred at step {self.rng.randint(0, start_step - 5)}, which was automatically corrected.",
                f"Routine data logging was interrupted briefly at step {self.rng.randint(0, start_step - 5)}, but no data was lost."
            ]
    ...
    def task_sensor_maintenance(
        ...
        """
        场景2: 传感器维护掉线 (借鉴CiK的SensorMaintenanceInPredictionTask)
        
        物理逻辑：设备计划内维护，导致某项指标在区间内强制归零或变为底噪
        ...
        """
        if start_step is None:
            start_step = self.rng.randint(int(self.seq_len * 0.4), int(self.seq_len * 0.6))
        if duration is None:
        ...
        prompt = (
            ...
            f"was initiated at time step {start_step} and completed by time step {end_step}. "
    ...
    def task_equipment_failure(
        ...
        failure_ratio = self.rng.uniform(0.1, 0.4)
        target_ts[start_step:end_step + 1, feat_idx] *= failure_ratio
        
        noise_intensity = self.rng.uniform(0.5, 2.0)
        target_ts[start_step:end_step + 1, feat_idx] += self.rng.normal(0, noise_intensity, duration)
        mask_gt[start_step:end_step + 1, feat_idx] = 1.0
        ...
        prompt = (
            f"[Context] An unexpected equipment failure occurred in the {feature_name} monitoring system "
            f"starting at time step {start_step}. The malfunction lasted for approximately {duration} steps, "
            f"causing erratic readings that dropped to about {failure_ratio:.0%} of normal values with significant noise. "
            f"The faulty component was replaced and normal operation resumed by step {end_step + 1}. "
            f"Please adjust the sequence to reflect this failure event."
        )
        
        if include_distractor:
            prompt += f" Note: A similar but minor glitch was observed at step {self.rng.randint(0, start_step - 5)}, which self-corrected."
            
        gt_params = {
            "task_type": "Equipment_Failure",
            "target_feature": target_feature,
            "start_step": start_step,
            "end_step": end_step,
            "failure_ratio": float(failure_ratio),
            "noise_intensity": float(noise_intensity),
            "causal_context": "equipment failure -> erratic readings -> component replacement",
            "has_distractor": include_distractor
        }
    ...
    def task_scheduled_shutdown(
        ...
        物理逻辑：计划内的系统停机，指标逐渐下降至零
        """
        if start_step is None:
            start_step = self.rng.randint(int(self.seq_len * 0.2), int(self.seq_len * 0.4))
        if duration is None:
        ...
        prompt = (
            f"[Context] A planned system shutdown was executed starting at time step {start_step}. "
    ...
    def task_load_balancing(
        ...
        if len(self.features) >= 2:
            feature_up = self.features[0]
            feature_down = self.features[1]
        else:
            feature_up = self.features[0]
            feature_down = self.features[0]
            
        feat_up_idx = self.feature_to_idx[feature_up]
        feat_down_idx = self.feature_to_idx[feature_down]
        end_step = min(start_step + duration - 1, self.seq_len - 1)
        
        transfer_ratio = self.rng.uniform(0.2, 0.4)
        target_ts[start_step:end_step + 1, feat_up_idx] *= (1 + transfer_ratio)
        target_ts[start_step:end_step + 1, feat_down_idx] *= (1 - transfer_ratio)
        
        mask_gt[start_step:end_step + 1, feat_up_idx] = 1.0
        mask_gt[start_step:end_step + 1, feat_down_idx] = 1.0
        
        prompt = (
            f"[Context] A load balancing operation was performed starting at time step {start_step} "
            f"and lasting for {duration} steps. Approximately {transfer_ratio:.0%} of the load was transferred "
            f"from {feature_down} to {feature_up} to optimize system efficiency. "
            f"Please adjust the sequence to reflect this load redistribution."
        )
        
        if include_distractor:
            prompt += f" Note: A similar balancing was attempted at step {self.rng.randint(0, start_step - 5)} but was aborted."
            
        gt_params = {
            "task_type": "Load_Balancing",
            "target_feature": feature_up,
            "secondary_feature": feature_down,
            "start_step": start_step,
            "end_step": end_step,
            "transfer_ratio": float(transfer_ratio),
            "causal_context": "load balancing -> load transfer -> efficiency optimization",
            "has_distractor": include_distractor
        }
    ...
    def generate_random_task(
        ...
        task_methods = {
            ...
            "equipment_failure": self.task_equipment_failure,
            "scheduled_shutdown": self.task_scheduled_shutdown,
            "load_balancing": self.task_load_balancing,
            "anomaly_injection": self.task_equipment_failure,
            "trend_reversal": self.task_load_balancing,
            "seasonal_adjustment": self.task_heatwave_overload
        }
...
class RealLLMClient:
    ...
    def generate_vague_prompt(
        ...
        """
        将CiK风格的高质量Prompt转换为模糊的宏观描述
        
        Args:
            cik_prompt: CiK风格的高质量Prompt
            temperature: 温度参数
...
class BetterTSETestPipeline:
    ...
    def __init__(
        ...
        self.cik_synthesizer = CiKStyleDataSynthesizer(
            seq_len=100,
    ...
    def run_batch_tests(
        ...
        Args:
            ...
            seq_len: 序列长度
            task_types: 任务类型列表
            include_distractor_ratio: 包含干扰项的比例
    ...
    def save_results(self, filename: str = "cik_test_results.json"):
        """保存测试结果"""
    ...
    def _save_report(self):
        """保存文本报告"""
        report_file = self.output_dir / "cik_evaluation_report.txt"
        
        summary = self._compute_summary()
        
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("BetterTSE 时间序列编辑测试评估报告 (CiK风格增强版)\n")
            f.write("=" * 70 + "\n\n")
            
            f.write(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"数据文件: {self.data_loader.csv_path}\n")
            f.write(f"样本数量: {len(self.samples)}\n")
            f.write(f"框架版本: BetterTSE-CiK Style\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='BetterTSE 时间序列编辑测试脚本 (CiK风格增强版)',
    ...
    log_file = Path(args.output_dir) / f"cik_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    logger.info("=" * 70)
    logger.info("BetterTSE 时间序列编辑测试框架 (CiK风格增强版)")
    logger.info("=" * 70)
