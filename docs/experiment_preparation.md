...
- Current student status is still experimental: the initial capacity ablation shows `mixed_capacity` is materially better than the plain linear head on ETTh1 heldout, but runtime and cross-distribution stability are not yet strong enough to promote the student into the locked pure-editing mainline.
